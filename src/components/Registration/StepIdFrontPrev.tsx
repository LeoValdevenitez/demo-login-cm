...
interface StepIdFrontPrevProps {
  onNext: () => void;
  onBack: () => void;
}

const StepIdFrontPrev: React.FC<StepIdFrontPrevProps> = ({ onNext, onBack }) => {
  const { registrationData } = useUserAuth();

  return (
    <div className="flex flex-col flex-1 items-center justify-between">
      <div className="text-center mb-4">
        <h2 className="text-xl font-bold text-gray-800 mb-2">Confirma el frente de tu documento</h2>
        <p className="text-gray-600">
          Asegúrate que todos los datos sean visibles y legibles
        </p>
      </div>

      <div className="flex-1 w-full flex items-center justify-center">
        {registrationData?.idFrontImage ? (
          <div className="relative w-5/6 max-w-md overflow-hidden rounded-lg border-4 border-[#e50046]">
            <img 
          ...
          <div className="bg-gray-200 w-5/6 max-w-md aspect-[3/2] rounded-lg flex items-center justify-center">
...
export default StepIdFrontPrev;